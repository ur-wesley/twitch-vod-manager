    ...
    <Card class="bg-card/60 backdrop-blur-xs">
      <CardContent class="p-3 space-y-1.5">
        ...
        <Show
          when={!props.loading}
          fallback={
            <span class="i-mdi-loading animate-spin size-4 text-primary" aria-hidden="true" />
          }
        >
          <Show
...
export const CloudLibrary: Component<CloudLibraryProps> = (props) => {
  ...
  const s3ListedBytes = () => s3List().reduce((acc, obj) => acc + obj.size_bytes, 0);
  ...
  return (
    <div class="space-y-4">
      ...
      >
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-card/60 border rounded-xl p-4 shadow-xs">
          <div class="space-y-0.5">
            ...
            <p class="text-xs text-muted-foreground">
              Total stored:{" "}
              <span class="font-medium text-foreground">{formatBytes(totalBytes())}</span>
            </p>
          </div>
        ...
        {/* S3 File List */}
        <Show
          when={props.s3Configured && provider() === "s3" && (!isLoading() || s3List().length > 0)}
        >
          <Show
        ...
        <Show
          when={
            props.gdriveConfigured &&
            provider() === "gdrive" &&
            (!isLoading() || gdriveList().length > 0)
          }
        >
          <Show
            ...
            fallback={
              <div class="p-12 text-center border rounded-xl bg-card/30 space-y-2">
                ...
                <p class="text-xs text-muted-foreground max-w-sm mx-auto">
                  Connect your Google Drive account in Settings and select Google Drive as an
                  archive destination.
                </p>
              </div>
            }
        ...
        <Show
          when={
            props.webdavConfigured &&
            provider() === "webdav" &&
            (!isLoading() || webdavList().length > 0)
          }
        >
          <Show
            ...
            fallback={
              <div class="p-12 text-center border rounded-xl bg-card/30 space-y-2">
                ...
                <p class="text-xs text-muted-foreground max-w-sm mx-auto">
                  Configure your Nextcloud, ownCloud, or NAS WebDAV endpoint in Settings and select
                  it as an archival target.
                </p>
              </div>
            }
          >
                      ...
                      <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                          ...
                          <div class="flex items-center gap-2 text-xs text-muted-foreground pt-0.5">
                            ...
                            <span
                              class="font-mono text-[10px] text-muted-foreground/60 truncate max-w-[140px]"
                              title={file.href}
                            >
                              {file.href}
                            </span>
