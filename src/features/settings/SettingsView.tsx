...
import type { AppSettings, FfmpegInfo, WorkerStatus } from "~/types";
...
export const SettingsView: Component<SettingsViewProps> = (props) => {
  ...
  const sampleToml = `# Twitch VOD Manager Sample Configuration
[twitch]
client_id = "your_twitch_client_id"
client_secret = "your_twitch_client_secret"

[s3]
# Options: "cloudflare_r2", "backblaze_b2", "custom"
provider = "cloudflare_r2"
endpoint = "https://<account_id>.r2.cloudflarestorage.com"
region = "auto"
bucket = "twitch-vod-archive"
access_key = "your_access_key_id"
secret_key = "your_secret_access_key"

[encoding]
# Options: "hevc_nvenc" (NVIDIA GPU), "libx265", "libx264", "passthrough"
encoder_preset = "hevc_nvenc"
crf = 24
output_dir = "C:\\\\Users\\\\Videos\\\\TwitchVODs"
...
[worker]
worker_url = "http://your-vps-ip:4000"
worker_api_key = "your_secret_bearer_key"
auto_archive_enabled = false
auto_archive_interval_mins = 15
`;
    ...
    <div class="flex h-full flex-col overflow-hidden bg-background font-sans">
      ...
      <div class="flex-1 overflow-hidden px-6 py-6">
        <div class="mx-auto w-full max-w-5xl h-full flex flex-col gap-6">
          ...
          >
            <TabsList class="mb-4 h-9 w-full shrink-0 flex justify-start bg-muted/40 p-1 rounded-lg border border-border/40">
              <TabsTrigger
                value="general"
                class="flex-1 text-xs font-semibold uppercase tracking-wider data-[selected]:bg-card data-[selected]:text-foreground data-[selected]:shadow-sm rounded-md transition-all"
              >
            ...
            <div class="w-full flex-1 overflow-y-auto pr-1 scrollbar-thin">
              ...
              {/* Tab 5: Accounts */}
              <TabsContent value="accounts" class="space-y-6 outline-none animate-in fade-in duration-200">
                <section class="space-y-4 rounded-xl border border-border/60 bg-card/40 p-5">
                  <div class="space-y-1">
                    <h3 class="text-sm font-bold uppercase tracking-wider text-primary">
                    ...
                    <p class="text-xs text-muted-foreground">
                      Configure OAuth client credentials to connect with Twitch and YouTube services.
                    </p>
                  </div>
                  ...
                  <div class="grid gap-4">
                    <div class="rounded-xl border border-border/50 bg-muted/20 p-4 space-y-3">
                      <div class="flex items-center gap-3">
                        <div class="flex size-9 items-center justify-center rounded-lg bg-[#9146FF]/20 text-[#9146FF]">
                          <span class="iconify mdi--twitch size-5" />
                        </div>
                        <div>
                          <p class="text-xs font-bold">Twitch Developer Credentials</p>
                          <p class="text-[10px] text-muted-foreground">
                            Obtain from dev.twitch.tv/console/apps (OAuth Redirect URL: http://localhost:8911/callback)
                          </p>
                        </div>
                      </div>
                    ...
                    <div class="rounded-xl border border-border/50 bg-muted/20 p-4 space-y-3">
                      <div class="flex items-center gap-3">
                        <div class="flex size-9 items-center justify-center rounded-lg bg-red-500/20 text-red-500">
                          <span class="iconify mdi--youtube size-5" />
                        </div>
                        <div>
                          <p class="text-xs font-bold">YouTube API Credentials (Optional)</p>
                          <p class="text-[10px] text-muted-foreground">
                            Google Cloud Console project OAuth credentials for automated 1-click publishing.
                          </p>
                        </div>
                      </div>
                        ...
                        <div class="space-y-1">
                          ...
                          <Show when={workerStatusData()}>
                            <p class="text-[11px] opacity-80">
                              ...
                              Disk Free: {workerStatusData()?.disk_free_gb.toFixed(1)}GB
                            </p>
                          </Show>
                        </div>
                      </div>
                  ...
                  <div class="space-y-2 pt-2">
                    <div class="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          navigator.clipboard.writeText(sampleToml);
                          toast.success("Sample TOML copied to clipboard!");
                        }}
